...
#         norm_layer_cp = norm_layer
#         self.norm_l = []
#         for i in range(len(self.depths)):
#             self.norm_l.append(norm_layer_cp[embed_dims[i]])
    def _init_weights(self, m):
        import math
        if isinstance(m, nn.Linear):
            trunc_normal_(m.weight, std=.02)
...
class PCPVT(CPVTV2):
    def __init__(self, img_size=[128,256], patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256],
