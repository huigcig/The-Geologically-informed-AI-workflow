...
public class LocalOrientFilterG {
   ...
   */
  public LocalOrientFilterG(double sigma1, double sigma2) {
   ...
   */
  public LocalOrientFilterG(double sigma1, double sigma2, double sigma3) {
    ...
    float[][] el)
  {
    ...
    // _rgfGradient1.apply10(ph,g1);
    // _rgfGradient2.apply01(ph,g2);
    for (int i2 = 0; i2 < n2; i2++) {
    for (int i1 = 0; i1 < n1; i1++) {
      //g1
      if (i1 == 0) {
          g1[i2][i1] = checkPi(ph[i2][i1 + 1] - ph[i2][i1]) * 0.5f;
      } else if (i1 == n1 - 1) {
          g1[i2][i1] = checkPi(ph[i2][i1] - ph[i2][i1 - 1]) * 0.5f;
      } else {
          float g1b = checkPi(ph[i2][i1 + 1] - ph[i2][i1    ]);
          float g1a = checkPi(ph[i2][i1    ] - ph[i2][i1 - 1]);
          g1[i2][i1] = (g1b+g1a) * 0.5f;
      }

      //g2
      if (i2 == 0) {
          g2[i2][i1] = checkPi(ph[i2 + 1][i1] - ph[i2][i1]) * 0.5f;
      } else if (i2 == n2 - 1) {
          g2[i2][i1] = checkPi(ph[i2][i1] - ph[i2 - 1][i1]) * 0.5f;
      } else {
          float g2b = checkPi(ph[i2 + 1][i1] - ph[i2    ][i1]);
          float g2a = checkPi(ph[i2    ][i1] - ph[i2 - 1][i1]);
          g2[i2][i1] = (g2b+g2a) * 0.5f;
      }
    }}

    //normalization
    float[][] gn = sqrt(add(pow(g1,2),pow(g2,2)));
    g1 = div(g1,gn);
    g2 = div(g2,gn);

    // //process g1,g2
    // float threshold = 0.3f*(float)Math.PI;
    // for (int i2 = 0; i2 < n2; i2++) {
    // for (int i1 = 0; i1 < n1; i1++) {
    //   if (Math.abs(g1[i2][i1]) > threshold) {
    //       g1[i2][i1] = 999.0f;
    //   }
    //   if (Math.abs(g2[i2][i1]) > threshold) {
    //       g2[i2][i1] = 999.0f;
    //   }
    // }}
    // BlendedGridder2 bg = new BlendedGridder2();
    // float[][] _1 = bg.gridNearest(999.0f,g1);
    // float[][] _2 = bg.gridNearest(999.0f,g2);
...
/**
   ...
   * @param el (eu-ev)/eu, a measure of linearity.
   */
  public void applyForGradient(float[][] x,
    float[][] g1, float[][] g2,  
    ...
    float[][][] ep, float[][][] el)
  {
    ...
    // Gradient.
    final float[][][] g1n = (nt>0)?t[0]:new float[n3][n2][n1];
    final float[][][] g2n = (nt>1)?t[1]:new float[n3][n2][n1];
    final float[][][] g3n = (nt>2)?t[2]:new float[n3][n2][n1];
    ...
    Parallel.loop(n3,new Parallel.LoopInt() {
      ...
      for (int i2 = 0; i2 < n2; i2++) {
      for (int i1 = 0; i1 < n1; i1++) {
        // g1: Gradient along the i1 (first) dimension
        if (i1 == 0) {
            g1n[i3][i2][i1] = checkPi(ph[i3][i2][i1 + 1] - ph[i3][i2][i1]) * 0.5f;
        } else if (i1 == n1 - 1) {
            g1n[i3][i2][i1] = checkPi(ph[i3][i2][i1] - ph[i3][i2][i1 - 1]) * 0.5f;
        } else {
            g1n[i3][i2][i1] = (checkPi(ph[i3][i2][i1 + 1] - ph[i3][i2][i1])
                              + checkPi(ph[i3][i2][i1] - ph[i3][i2][i1 - 1])) * 0.5f;
        }

        // g2: Gradient along the i2 (second) dimension
        if (i2 == 0) {
            g2n[i3][i2][i1] = checkPi(ph[i3][i2 + 1][i1] - ph[i3][i2][i1]) * 0.5f;
        } else if (i2 == n2 - 1) {
            g2n[i3][i2][i1] = checkPi(ph[i3][i2][i1] - ph[i3][i2 - 1][i1]) * 0.5f;
        } else {
            g2n[i3][i2][i1] = (checkPi(ph[i3][i2 + 1][i1] - ph[i3][i2][i1])
                              + checkPi(ph[i3][i2][i1] - ph[i3][i2 - 1][i1])) * 0.5f;
        }

        // g3: Gradient along the i3 (third) dimension
        if (i3 == 0) {
            g3n[i3][i2][i1] = checkPi(ph[i3 + 1][i2][i1] - ph[i3][i2][i1]) * 0.5f;
        } else if (i3 == n3 - 1) {
            g3n[i3][i2][i1] = checkPi(ph[i3][i2][i1] - ph[i3 - 1][i2][i1]) * 0.5f;
        } else {
            g3n[i3][i2][i1] = (checkPi(ph[i3 + 1][i2][i1] - ph[i3][i2][i1])
                              + checkPi(ph[i3][i2][i1] - ph[i3 - 1][i2][i1])) * 0.5f;
        }
      }}}
    });

    //normalization
    // float[][][] gn = sqrt(add(add(pow(g1n,2),pow(g2n,2)),pow(g3n,2)));
    // float[][][] g1 = div(g1n,gn);
    // float[][][] g2 = div(g2n,gn);
    // float[][][] g3 = div(g3n,gn);
